...
PvIB or Platform voor InformatieBeveiliging is a knowledge center that aims to gather and spread knowledge about information security. It achieves this goal via organising events to share knowledge, cooperation with parties in the industry, and publishing news about the industry among others. To learn more about PvIB itself visit: [pvib.nl][pvib.nl].
...
With now having a 1: and a 5: it started to look like there were five or more hidden pieces to be recovered, so I started to look deeper into the PDF.

Running the tool `pdfparser` within Kali revealed the 4th bit of information within the Author field that looked like so:
...
The next challenge Plzhackme linked to a website showing plain HTML text about the server and that present on it. The website would look the following from within a browser:

![PvIB CTF Plzhackme challenge showing the homepage of the challenge](/images/CTF/PvIB/PvIB-CTF-2022-Plzhackme-homepage.webp)

This shows that the flag we need to grab is present within the machine under the path `/opt/tools/flag.txt` and will be our main priority. As for getting the flag, the website contents explicitly hints at the creator hoping the webserver is secure. Looking at the response given by the webserver shows the webserver as Apache 2.4.49:
...
Looking online for this version of Apache reveals a CVE for path traversal a remote code execution (RCE) with a matching exploit. This exploit is available via exploit db under following URL: [exploit-db.com/exploits/50383][exploitdb-50383] and contains the following shell script:
...
A follow-up to Plzhackme was also present in the form of Plzhackme2. This challenge showed an updated website acknowledging the shortcomings of the former version:
...
The server version has incremented by one from 2.4.49 to 2.4.50! A brief look at the prior exploit shows that an adapted version exists for 2.4.50 that can be found at exploit db under: [exploit-db.com/exploits/50406][exploitdb-50406] and contains the same shell script more or less:
...
The last challenge that I worked on and completed with Jurrien was a challenge named Really Secure Authentication. This challenge was about secure communications that had been intercepted in an attached text file. When opening the text file the following three lines of text were visible:
...
The format of data looked familiar to Jurrien and I and he then found a Stack Overflow thread about decrypting RSA cipher text when you have n, e, and c. The thread links to a Python script to crack RSA CTF challenges, but this ended up taking too long to be practical from the short few minutes we tried it. An old favourite of mine, [dcode.fr][dcodefr] also linked within the thread and ended up giving us the flag.

[dcode.fr][dcodefr] is a neat website for encrypting and decrypting cipher texts found in CTFs or other challenge formats. It contains algorithms for a broad range of ciphers and even have RSA it seems!
...
[exploitdb-50406]:    https://www.exploit-db.com/exploits/50406
[dcodefr]:            https://dcode.fr